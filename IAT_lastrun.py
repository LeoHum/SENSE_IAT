...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    image = visual.ImageStim(
        ...
        ori=0.0, pos=(0, -0.4), size=(0.8, 0.2),
        color=[1,1,1], colorSpace='rgb', opacity=None,
        flipHoriz=False, flipVert=False,
        texRes=128.0, interpolate=True, depth=-2.0)
    ...
    for thisBlocks_test in blocks_test:
        ...
        for thisTrials_test in trials_test:
            ...
            # keep track of which components have finished
            trial_testComponents = [fixation_2, text_stim_2, keyboard_resp_2, error_msg_2, rightcat_4, leftcat_4]
            ...
            # update component parameters for each repeat
            # keep track of which components have finished
            transition_testComponents = [cross_2]
    ...
    # keep track of which components have finished
    score2Components = [score_meaning_2, end_score_2, image]
    ...
    while continueRoutine:
        ...
        if end_score_2.status == NOT_STARTED and t >= 0.1-frameTolerance:
            # keep track of start time/frame for later
            ...
            # keyboard checking is just starting
            end_score_2.clock.reset()  # now t=0
        if end_score_2.status == STARTED:
            theseKeys = end_score_2.getKeys(keyList=['space',], ignoreKeys=["escape"], waitRelease=False)
            _end_score_2_allKeys.extend(theseKeys)
            if len(_end_score_2_allKeys):
                end_score_2.keys = _end_score_2_allKeys[-1].name  # just the last key pressed
                end_score_2.rt = _end_score_2_allKeys[-1].rt
                end_score_2.duration = _end_score_2_allKeys[-1].duration
                # was this correct?
                if (end_score_2.keys == str("'space'")) or (end_score_2.keys == "'space'"):
                    end_score_2.corr = 1
                else:
                    end_score_2.corr = 0
                # a response ends the routine
